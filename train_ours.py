...
if not os.path.exists("/ssd_scratch/cvit/anirudhkaushik/data_train.npy"):
    ...
    # save all data
    data_array = np.array([data_train, data_test, glove_train, glove_test, data_fine, data_fine_test, glove_fine, glove_fine_test])
    np.save("/ssd_scratch/cvit/anirudhkaushik/data_train.npy",data_array)
else:
    data_array = np.load("/ssd_scratch/cvit/anirudhkaushik/data_train.npy", allow_pickle=True)
    data_train = data_array[0]
    data_test = data_array[1]
    glove_train = data_array[2]
    glove_test = data_array[3]
    data_fine = data_array[4]
    data_fine_test = data_array[5]
    glove_fine = data_array[6]
    glove_fine_test = data_array[7]
...
for i in range(5):
    ...
    model_mean.fit([data_fine], [data_fine,glove_fine,class_fine,mean_tot, mean_den_tot], batch_size=batch_size, epochs=epochs, verbose=2, callbacks=callback_list, validation_data=([data_fine_test], [data_fine_test, glove_fine_test,class_fine_test,mean_test, mean_den_test]))   
    rec, glove, clas, pred, pred_den = model_mean.predict(data_fine)
