...
save_file =str(os.path.dirname(os.path.abspath(__file__)))+'/model_weights/Subject' +subject+'_big_model.h5'
...
model = encdec_big_model(concating=False)
model_mean = encdec_big_model(concating=True)
...
model_mean.compile(loss= {'pred_glove':mean_distance_loss,'pred_class':categorical_crossentropy, 'Concat':mean_distance_loss},loss_weights=[glove_weight,class_weight, 1.0],optimizer=optimizer, metrics=['accuracy']) 
...
glove, clas, pred = model_mean.predict(data_fine)
...
for i in range(5):
    ...
    model_mean.fit([data_fine], [glove_fine,class_fine,mean_tot], batch_size=batch_size, epochs=50, verbose=2, callbacks=callback_list, validation_data=([data_fine_test], [glove_fine_test,class_fine_test,mean_test]))   
    glove, clas, pred = model_mean.predict(data_fine)
...
glove_pred_test, pred_class, pred_mean = model_mean.predict(data_fine_test)

text_all = open(str(os.path.dirname(os.path.abspath(__file__))) + "/results/Subject" +subject+"_big_model.txt", "a+")
